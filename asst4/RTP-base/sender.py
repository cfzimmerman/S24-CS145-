...
from typing import Deque, Tuple, Optional, Union
...
class RtpSender:
    ...
    def __init__(self, window_size: int, receiver_ip: str, receiver_port: int):
        ...
        self.__in_flight: Deque[Tuple[int, bytes]] = deque(maxlen=window_size)
        # an open device socket used for RTP I/O
        self.__socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        self.__socket.settimeout(self.TIMEOUT_MS / 1000)
    ...
    def send(self, payload: Union[bytes, str]) -> None:
        ...
        while True:
            ...
            end = min(start + self.PAYLOAD_MAX_BYTES + 1, len(payload))
            if start == end:
                break
            self.__send_queue.append(payload[start:end])
            start = end
        self.__manage_window()

    def close(self):
        '''Processes any buffered data and then requests to close
        the connection. Forcibly exits after TIMEOUT_MS if the
        receiver fails to acknowledge the END request.'''

        self.__manage_window()

        self.__send_pkt_unchecked(
            pkt_type=PacketType.END, payload=bytes(0), seqno=self.__curr_seqno)
        self.__curr_seqno += 1
        ...
        while True:
            ...
            try:
                ...
                break
            if ack is not None and ack.seq_num == self.__curr_seqno:
                break
        self.__socket.close()
    ...
    def __manage_window(self):
        ...
        while 0 < len(self.__send_queue) + len(self.__in_flight):
            while (0 < len(self.__send_queue) and
                ...
                self.__in_flight.append((self.__curr_seqno, payload))
                self.__curr_seqno += 1
            ...
            try:
                ...
                # on timeout, resend everything in the window
                for seqno, payload in self.__in_flight:
                    self.__send_pkt_unchecked(PacketType.DATA, payload, seqno)
                continue
            if ack is None:
                # ack was corrupted, ignore it
                continue

            # pop from in-flight everything up to the new ack number
            while (0 < len(self.__in_flight) and
                   self.__in_flight[0][0] < ack.seq_num):
