...
subprocess.Popen("sudo killall iperf iperf3", shell=True,
...
for i, port in enumerate(used_ports):
    Popen(iperf_send.format("h1", topo.get_host_ip("h2"), duration,
          topo.get_host_ip("h1"), port, c_ports[i], i), shell=True)
