...
def usage():
    print(
        ...
        Insert P4 table entries to route traffic among hosts for \
        FatTree topology with [k] value")
...
class RoutingController(object):
    ...
    def __init__(self):
        ...
        try:
            ...
            print("Failed to open the topology database file \
            'topology.json'. Did you run the network with \
            'p4run'?\n\tCause: {}".format(e))
            exit(1)
        self.controllers = {}
        self.init()
    ...
    def route(self):
        ...
        for sw_name, controller in self.controllers.items():
            for (dmac, port) in MultiCoreRoutes.get_routes(topo, sw_name):
